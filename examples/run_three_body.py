...
for t in range(len(positions_hist)):
    pos = positions_hist[t]
    vel = velocities_hist[t]
    # min pairwise distance
    dmin = np.min([np.linalg.norm(pos[i]-pos[j]) for i in range(len(pos)) for j in range(i+1, len(pos))])
...
plt.title("Total Energy vs Time (Three Body, Newtonian vs Yukawa)")
plt.xlabel("Timestep")
plt.ylabel("Energy")
plt.legend()
plt.show()
